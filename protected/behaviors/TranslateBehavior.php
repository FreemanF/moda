<?php
/**
 * TranslateBehavior class file
 *
 * @author guillemc, Frédéric Rocheron<frederic.rocheron@gmail.com>
 */
/**
 * TranslateBehavior handles active record model translation.
 *
 ...
 *     return array(
 *         'ml' => array(
 *             'class' => 'application.models.behaviors.TranslateBehavior',
...
class TranslateBehavior extends CActiveRecordBehavior
{
     ...
     */
    //public $langClassName = 'Translate';
     ...
     */
    //public $langTableName = '{{translate}}';
    
    /**
     * @var integer the name of the foreign key field of the translation table related to base model table. Default to '[base model table name]_id'.
     * Example: for a base model table named post', the translation model table name is 'post_id'. 
     */
    //public $langForeignType = 'object_type';
    //public $langForeignKey  = 'object_id';
    ...
    public $metaField=false; // Добавить метатеги
    ...
    private static $mlAlias = 0;
    private $localizedLang = '';
    private $_ownerClass;
    private $_objectType;
    ...
    private function bindLocalizedAttributes($force=false) {
        if ($this->localizedLang)
        foreach($this->localizedAttributes as $name) {
            $langfield = $name.'_'.$this->localizedLang;
            if (isset($this->_langAttributes[$langfield]))
                $this->_langAttributes[$name] = &$this->_langAttributes[$langfield];
            elseif ($force) {
                $this->_langAttributes[$langfield]=null;
                $this->_langAttributes[$name] = &$this->_langAttributes[$langfield];
            }
        }
    }
    
    // Чтобы не конфликтовать с PrefixedBehavior.getOwnerClass
    private function getObjectClass($owner=null) {
        if (is_null($this->_ownerClass)) {
            if (is_null($owner)) $owner = $this->owner;
            $this->_ownerClass = get_class($owner);
        }
        return $this->_ownerClass;
    }
    
    // Чтобы не конфликтовать с PrefixedBehavior.getObjectType
    private function getOwnerType($owner=null) {
        if (is_null($this->_objectType)) {
            $class = $this->getObjectClass($owner);
            $obj = Object::byAlias($class);
            $this->_objectType = $obj->oid;
        }
        return $this->_objectType;
    }
    
    private function createLocalizedRelation($owner, $lang) {
        //Yii::log("createLocalizedRelation:$lang");
        $this->localizedLang = $lang;
        $rel   = $this->localizedRelation;
        $type  = $this->getOwnerType($owner);
        $class = CActiveRecord::HAS_MANY;
        $owner->getMetaData()->relations[$rel] = 
            new $class($rel, 'Translate', 'object_id', 
            array('on' => "$rel.object_type=$type and $rel.lang='$lang'", 'index' =>'name'));
        $this->bindLocalizedAttributes(true);
        return $type;
    }
    ...
    public function attach($owner) {
        ...
        if (is_null($this->defaultLanguage))
            $this->defaultLanguage = Yii::app()->language; //Yii::app()->params['defaultLanguage'];
        if (is_null($this->languages))
            $this->languages = Yii::app()->params['translatedLanguages'];
        if (array_values($this->languages) !== $this->languages) //associative array
            $this->languages = array_keys($this->languages);

        if ($this->metaField) {
            $this->localizedAttributes[] = 'e_title';
            $this->localizedAttributes[] = 'e_keywords';
            $this->localizedAttributes[] = 'e_description';
        }
        
        $class = CActiveRecord::HAS_MANY;
        $objectType = $this->createLocalizedRelation($owner, Yii::app()->language);
        $rel = $this->multilangRelation;
        $owner->getMetaData()->relations[$rel] = 
            new $class($rel, 'Translate', 'object_id', array(
            'on' =>"$rel.object_type=$objectType",
    ...
    public function nextAliases($suffix=null) {
        
        if (is_null($suffix)) $suffix = self::$mlAlias++;
        
        $meta = $this->owner->getMetaData();
        $type = $this->getOwnerType();
        if (isset($meta->relations[$this->localizedRelation])) {
            $rel  = $this->localizedRelation.$suffix;
            $lang = $this->localizedLang;
            $meta->relations[$this->localizedRelation]->alias = $rel;
            $meta->relations[$this->localizedRelation]->on = 
                "$rel.object_type=$type and $rel.lang='$lang'";
        }
        if (isset($meta->relations[$this->multilangRelation])) {
            $rel = $this->multilangRelation.$suffix;
            $meta->relations[$this->multilangRelation]->alias = $rel;
            $meta->relations[$this->multilangRelation]->on = "$rel.object_type=$type";
        }
    }
     ...
     * @return array 
     */
    public function localizedCriteria($scope=array()) {
        if (!isset($scope['with'])) $scope['with'] = array();
        elseif (!is_array($scope['with'])) $scope['with'] = array($scope['with']);
        $scope['with'][$this->localizedRelation] = array();
        return $scope;
    }
     ...
     * @return array 
     */
    public function multilangCriteria($scope=array()) {
        if (!isset($scope['with'])) $scope['with'] = array();
        elseif (!is_array($scope['with'])) $scope['with'] = array($scope['with']);
        $scope['with'][$this->multilangRelation] = array();
        return $scope;
    }
    ...
    public function hasLangAttribute($name) {
        $has = array_key_exists($name, $this->_langAttributes);
        //Yii::log("hasLangAttribute($name):".($has?'true':'false'));
        return $has;
    }
    ...
    public function afterConstruct($event) {
        ...
        $this->bindLocalizedAttributes();
        //Yii::log("afterConstruct");
    }
...
//        $criteria = new CDbCriteria($criteria_array);
    ...
    public function afterFind($event) {
        ...
        if ($owner->hasRelated($this->multilangRelation)) {
            $related = array();
            foreach($owner->getRelated($this->multilangRelation) as $tr)
                $related[$tr->lang][$tr->name] = $tr;
            foreach ($this->languages as $lang)
                foreach ($this->localizedAttributes as $field)
                    $this->setLangAttribute($field. '_' . $lang, isset($related[$lang][$field]) ? $related[$lang][$field]->value : null);
        } else if ($owner->hasRelated($this->localizedRelation)) {
            $related = $owner->getRelated($this->localizedRelation);
            foreach ($this->localizedAttributes as $field) {
                if (!empty($related[$field]))// || $this->forceOverwrite)
                    $owner->$field = $related[$field]->value;
                else 
                    $owner->$field = '';
            }
    ...
    public function afterSave($event) {
        ...
        $ownerId = $main_owner->getPrimaryKey();
        $rs = array();
        if (!$main_owner->isNewRecord) {
            foreach(Translate::model()->findAll("object_type=$ownerType and object_id=$ownerId") as $tr) 
                $rs[$tr->lang][$tr->name] = $tr;
        }
//        Yii::log('afterSave:'.var_export($this->_langAttributes, true));
//        Yii::log('owner->m_name_ru:'.$main_owner->m_name_ru);
//        Yii::log('owner->m_name_en:'.$main_owner->m_name_en);
        foreach ($this->languages as $lang) {
            ...
            foreach ($this->localizedAttributes as $field) {
                ...
                if (isset($rs[$lang][$field])) {
                    $owner = $rs[$lang][$field];
                    if ($this->forceDelete)
                    if (is_null($value) || $value==='') {
                        $owner->delete();
                        $owner = null;
                    } elseif($owner->value == $value)
                        $owner = null;
                } elseif (!$this->forceDelete || (!is_null($value) && $value!=='')) {
                    $owner = new Translate();
                    $owner->object_type = $ownerType;
                    $owner->object_id = $ownerId;
                    $owner->lang = $lang;
                    $owner->name = $field;
                    $owner->value= '';
                } else
                ...
                if($owner && ($this->forceDelete || (!is_null($value) && $owner->value != $value))) {
                    $owner->value = $value;
                    $owner->save(false);
                }
            }
    ...
    public function afterDelete($event) {
        if ($this->forceDelete) {
            ...
            $ownerType = $this->getOwnerType();
            $ownerId = $main_owner->getPrimaryKey();
            Translate::model()->deleteAll("object_type=$ownerType and object_id=$ownerId");
        }
    }
    ...
    {
        $has = parent::hasProperty($name) or $this->hasLangAttribute($name);
        //Yii::log("hasProperty($name):".($has?'true':'false'));
        return $has;
    }
    
    public function getMetaStuff() {
        if ( ! $this->metaField)
            throw new Exception('property TranslateBehavior.metaField is not true');
        $meta = array(
            'e_title'       => $et=$this->owner->e_title,
            'e_keywords'    => $ek=$this->owner->e_keywords,
            'e_description' => $ed=$this->owner->e_description,
        );
        if (!($et || $ek || $ed))
            $meta = null;
        return $meta;
