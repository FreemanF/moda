	...
	class watermark
	{
		...
		public static function output ($input, $output = null, $options = null)
		{
			...
			static $defOptions = array(
				'watermark' 	=> 'protected/components/watermark.png',
		...
		private static function _render ($input, $options)
		{
			...
			$starWatermark = self::_imageCreate($options['watermark']);
			if ( ! is_resource($starWatermark) ) {
			...
			$watermark_width =  imagesx($starWatermark); 
			$watermark_height =  imagesy($starWatermark);
                        
                        if( (($watermark_width+$options['hshift']) > $image_width) ||
                            (($watermark_height+$options['vshift']) > $image_height)){
                            if( $watermark_height > $watermark_width){
                                $exportY = $image_height+$options['vshift'];
                                $exportX = floor($watermark_width*$exportY/$watermark_height);
                            }else{
                                $exportX = $image_width+$options['hshift'];
                                $exportY = floor($watermark_height*$exportX/$watermark_width);
                            }
                            $watermark = imagecreatetruecolor($exportX, $exportY);
                            $alphabg = imagecolorallocatealpha($watermark, 0, 0, 0, 127);
                            imagefill($watermark, 0, 0, $alphabg);
                            imagealphablending($watermark, false);
                            imagesavealpha($watermark, true);
                            imagecopyresampled($watermark, $starWatermark, 0, 0, 0, 0, $exportX, $exportY, $watermark_width, $watermark_height);
                            $watermark_width =  $exportX; 
                            $watermark_height =  $exportY;
                        }else{
