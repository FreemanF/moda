...
class elFinder {
	...
	protected function _rename()
	{
		...
		||  false == ($target = $this->_find(trim($_GET['target']), $dir))
			...
			$this->_rmTmb($target);
			$this->_logContext['from'] = $this->_trimDir($target);
			$this->_logContext['to']   = $this->_trimDir($dir.DIRECTORY_SEPARATOR.$name);
	...
	protected function _mkdir()
	{
		...
		$this->_logContext['dir'] = $this->_trimDir($dir.DIRECTORY_SEPARATOR.$_GET['name']);
		if (!$this->_isAllowed($dir, 'write')) {
			...
			$this->_logContext['dir'] = $this->_trimDir($dir.DIRECTORY_SEPARATOR.$name);
			$this->_result['select']  = array($this->_hash($dir.DIRECTORY_SEPARATOR.$name));
	...
	protected function _mkfile()
	{
		...
		$this->_logContext['file'] = $this->_trimDir($dir.DIRECTORY_SEPARATOR.$_GET['name']);
		if (!$this->_isAllowed($dir, 'write')) {
			$this->_result['error'] = 'Access denied';
		} elseif (false == ($name = $this->_checkName($_GET['name'])) ) {
			$this->_result['error'] = 'Invalid name';
			...
			$this->_logContext['file'] = $this->_trimDir($f);
	...
	protected function _rm()
	{
		...
		foreach ($_GET['targets'] as $hash) {
			if (false != ($f = $this->_find($hash, $dir))) {
				$this->_remove($f);
				$this->_logContext['targets'][] = $this->_trimDir($f);
			}
		}
	...
	protected function _upload()
	{
		...
		$this->_logContext['watermark'] = false;
		$this->_result['select'] = array();
		$total = 0;
		for ($i=0, $s = count($_FILES['upload']['name']); $i < $s; $i++) {
			if (!empty($_FILES['upload']['name'][$i])) {
				$total++;
				$this->_logContext['upload'][] = $this->_trimDir($dir.DIRECTORY_SEPARATOR.$_FILES['upload']['name'][$i]);
				if ($_FILES['upload']['error'][$i] > 0) {
					...
					if (!@move_uploaded_file($_FILES['upload']['tmp_name'][$i], $file)) {
						...
						@chmod($file, $this->_options['fileMode']);
                                                if (isset($_POST['add_watermark'])){
                                                    $this->_logContext['watermark'] = true;
                                                    @watermark::output ($file,$file);
                                                }
	...
	protected function _paste()
	{
		...
		$this->_logContext['dest'] = $this->_trimDir($dst);
		...
		foreach ($_GET['targets'] as $hash) {
			...
			$this->_logContext['src'][] = $this->_trimDir($f);
	...
	protected function _trimDir($name)
	{
            return str_replace($this->_options['root'], '', $name);
	}
