...
/*    $.post(url, response, function(output) 
	...
	$.post(
                '<?php echo Yii::app()->getBaseUrl(true) ?>/site/fblogin/',
        {status:'login',userData: JSON.stringify(response)},
        function(data){
//            console.log(data);
//
...
<div id="status">
			...
			<br><p>Для зарегистрированных пользователей</p><br>
                <a  class="b-auth__button b-button b-button_fs_18 b-button_fw_normal b-button_bg_gray"
                    href="<?php echo Yii::app()->getBaseUrl(true); ?>/users/login/">Войти через логин и пароль</a>
            </div>
        </div>
    </div>
